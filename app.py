...
from backend.gmail_loader import get_latest_emails
...
page = st.sidebar.radio(
    "Go to", ["Inbox Loader", "Prompt Brain", "Email Agent", "Draft Manager", "About"]
)
...
if page == "Inbox Loader":
    st.subheader("Load Real Gmail Inbox")

    num = st.number_input(
        "How many latest emails to load?", min_value=1, max_value=50, value=20
    )

    if st.button("Connect Gmail & Load"):
        with st.spinner("Connecting to Gmail..."):
            try:
                emails = get_latest_emails(num)
                db.save_json(DATA_DIR / "mock_inbox.json", emails)
                st.success(f"Loaded {len(emails)} emails from your inbox.")
                st.experimental_rerun()
    ...
    if not mock_inbox:
        st.info(
            "Inbox empty. Use Prompt Brain to load sample, or upload mock_inbox.json."
        )
    else:
        import pandas as pd
            ...
            [
                {
                    ...
                    "category": processed.get(str(e.get("id", idx)), {}).get(
    ...
    st.markdown("---")
    st.button(
        "Run ingestion (categorize & extract actions) 🔁",
        on_click=lambda: ingestion.run_ingestion(str(DATA_DIR), prompts),
    )
...
# --- Page: Prompt Brain ---
    ...
    st.markdown(
        "Edit and save the prompts which will be used by the agent on all operations."
    )
    with st.form("prompts_form"):
        ...
        tone = st.text_area(
            "Optional Tone Prompt (example: friendly/concise)",
...
# --- Page: Email Agent ---
    ...
    if not inbox:
        ...
        question = st.text_input(
            "Your instruction (e.g. 'Summarize this email', 'What tasks do I need to do?', 'Draft a reply in a friendly tone')",
            value="",
        )
        if st.button("Run"):
